...
struct t_SpiDebug* ptSpiReadWriteTelSingle(struct t_SpiTransfer* ptTransferBlock)
{
	...
	ptSpiTel->intRet = ioctl(ptTransferBlock->intFd, TIOMESSAGE, &tTransfer);
...
struct t_SpiDebug* ptSpiReadWriteTelMulti(struct t_SpiTransfer* ptTransferBlock)
{
	...
	// Start condition
	u32StartAddrIdx = ptTransferBlock->u32Address;
	u16Zyklus = 0;

	// Splintered transfer
	for (int i = 0; i < (ptTransferBlock->u16Length); i++)
	{
		au8TelTx[u8Idx + CPLD_DATA_OFFSET_UART] = *(ptTransferBlock->pu8DataOut + i);
		u8Idx = (u8Idx + 1) & 0x7F;
		if (u8Idx >= CPLD_MAX_LEN)
		{
			...
			tTransfer.tx_buf = (unsigned long)au8TelTx;
			tTransfer.rx_buf = (unsigned long)au8TelRx;
			tTransfer.len = (uint32_t)(CPLD_MAX_LEN + CPLD_DATA_OFFSET_UART);
			tTransfer.delay_usecs = SPI_DELAY;
			tTransfer.speed_hz = ptTransferBlock->u32Freq;
			tTransfer.bits_per_word = SPI_BITS;
	...
	if (u8Idx > 0)
	{
		...
		u32StartAddrIdx = u32StartAddrIdx << 1;

		u32StartAddrIdx = (uint32_t)(((u32StartAddrIdx & 0xFF0000) >> 16) | ((u32StartAddrIdx & 0xFF) << 16) | (u32StartAddrIdx & 0xFF00));

		memcpy(&au8TelTx[0], &u32StartAddrIdx, 3);

		if (ptTransferBlock->bRead == TRUE)
		{
			au8TelTx[2] |= READ;
			au8TelTx[2 + u8Idx] = 0x01;
			au8TelTx[3 + u8Idx] = 0x01;
		}
		else
		{
			au8TelTx[3] = 0;
		...
		tTransfer.tx_buf = (unsigned long)au8TelTx;
		tTransfer.rx_buf = (unsigned long)au8TelRx;
		tTransfer.len = (uint32_t)(u8Idx + CPLD_DATA_OFFSET_UART);
		tTransfer.delay_usecs = SPI_DELAY;
		tTransfer.speed_hz = ptTransferBlock->u32Freq;
		tTransfer.bits_per_word = SPI_BITS;
		...
		{
			*(ptTransferBlock->pu8DataIn + idx + (CPLD_MAX_LEN * u16Zyklus)) = au8TelRx[idx + CPLD_DATA_OFFSET_UART];
		}
	}
