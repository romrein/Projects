...
int iSpiTransferTest(int intFD, uint32_t u32ZykCnt, uint16_t u16Address, uint8_t u8UserLen, uint32_t u32Speed)
{
	...
	uint8_t au8LenDebug[130];								// Debugging buffer for the length analysis
	uint8_t au8AddressDebug[1024];							// Debugging buffer for the adress analysis
	uint8_t au8BufTx[1024];									// Debugging buffer for the transfer memory arrea
	uint8_t au8BufRxR[1024];								// Debugging buffer for the receive memory arrea
	uint8_t au8TelTx[130];									// Transfer buffer (TX)
	uint8_t au8TelRx[130];									// Receive buffer (RX)
...
int iSpiTransferTestSingle(int intFD, uint32_t u32ZykCnt, uint16_t u16Address, uint8_t u8UserLen, uint32_t u32Speed)
{
	...
	uint8_t u8LenIdx = 0;										// Data length. Note: not the telegram length
	uint8_t au8TelTxTmp[130];									// Temporal transfer buffer
	uint8_t au8BufRxR[1024];									// Debugging buffer for the receive memory arrea
	uint8_t au8TelTx[130];										// Transfer buffer (TX)
	uint8_t au8TelRx[130];										// Receive buffer (RX)
